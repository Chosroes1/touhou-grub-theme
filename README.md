...
====
1.在[Release](https://github.com/Chosroes1/touhou-grub-theme/releases/tag/v1.0)中选择自己屏幕分辨率对应的压缩包。
...
```
3.编辑`/etc/default/grub`，找到`GRUB_TERMINAL_OUTPUT=console`,把console改为gfxterm
  找到或添加GRUB_THEME=，取消注释，并添加主题路径。`GRUB_THEME="/boot/grub/themes/th-cb/theme.txt"`
  保存并退出。
4.更新grub。
``` bash
